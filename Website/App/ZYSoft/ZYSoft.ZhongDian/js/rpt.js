...
var BILL = 1, REPRINT = 2, STOCK = 4, SPECIAL = 5;
var RPTID = 1
var self = (vm = new Vue({
    el: "#app",
    data: function () {
    ...
    methods: {
        ...
        openTab4Dialog(row) {
            var total = row[0].iQuantity;
            if (total <= 0) {
                return layer.msg("当前行的数量为0", {
                    zIndex: new Date() * 1,
                    icon: 5,
                });
            }
            openDialog({
                title: "设置包装数量 总计:" + total,
                url: "./modalTab4/ModalTab4.aspx",
                area: ["600px", "500px"],
                onSuccess: function (layero, index, layer) {
                    layer.setTop(layero);
                    var iframeWin = window[layero.find("iframe")[0]["name"]];
                    iframeWin.init({ parent: self, sourceRow: row[0] });
                },
                onBtnYesClick: function (index, layero, layer) {
                    var iframeWin = window[layero.find("iframe")[0]["name"]];
                    var row = iframeWin.getSelect();
                    if (row != void 0 && row.length > 0) {
                        self.doSend(PREVIEW, row.map(function (f) {
                            return {
                                FEntryID: f.EnryID,
                                FPackQty: f.qty,
                                FBatch: f.cBatch,
        ...
        doPreview() {
            ...
            if (this.activeName == 'tab4') {
                data = this.grid4.getSelectedData()
                if (data.length > 1) {
                    return layer.msg("只允许选择一行", {
                        zIndex: new Date() * 1,
                        icon: 5,
                    });
                }
                this.openTab4Dialog(data)
            } else {
            ...
            if (socket && socket.readyState == "1") {
                socket.send(JSON.stringify(
                    $.extend({},
                        {
                            FType: flag,
                            FBillType: self.activeName == 'tab1' ? BILL :
                                (self.activeName == 'tab2' ? REPRINT : (self.activeName == 'tab3' ? STOCK : SPECIAL)),
                            FAccountID: accounId
                        },
                        { Entry: data }
                    )))
        ...
        initGrid(opt) {
            ...
            this[gridId] = new Tabulator("#" + gridId, {
                ...
                data: [],
                selectable: true,
                selectableRows: gridId == 'grid4' ? 1 : true,
                ajaxResponse: function (url, params, response) {
                    if (response.state == "success") {
                        var t = response.data.map(function (m, i) {
                            if (self.activeName == 'tab1' || self.activeName == 'tab4') {
        ...
        handleClick(tab, event) {
            ...
            if (self.activeName == 'tab4') {
                var ps = Object.keys(self.grid4);
                if (ps.length <= 0) {
                    this.initGrid({
                        gridId: "grid4",
                        key: "id",
    ...
    mounted() {
        ...
        window.onresize = function () {
            ...
            var ps4 = Object.keys(self.grid4);
            if (ps4.length > 0) {
                self.grid4.setHeight(
                    $(window).height() -
                    $("#toolbarContainer").height() -
                    85
                );
